	...
	for _, rootpath in pairs(RootPath) do
		...
		for i = 0, len-1 do
            ...
            if line ~= nil then
                ...
                table.insert(HU.FileMap[line], {SysPath = line, LuaPath = luapath})
            end
	    end
	    -- file:close()
	end
...
function HU.UpdateAllFunction(OldTable, NewTable, Name, From, Deepth, LuaPath)
	...
	if tostring(OldTable) == nil or tostring(NewTable) == nil then
		-- logerror(utils.serialize(OldTable))
		return
	end
	...
	for ElementName, Element in pairs(NewTable) do
		...
		if type(Element) == type(OldElement) then
			...
			elseif type(Element) == "string" or "number" then
	...
	if LuaPath and string.find(LuaPath,"Core/ClientData") then
