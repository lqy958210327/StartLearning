...
local LuaProfiler = CS.LuaProfiler
local LuaToolkit = CS.Framework.Tools.LuaToolkit
local IS_EDITOR_OR_DEBUG = IS_EDITOR or not LuaToolkit.IsPublishVersion()

local start_time

local function create_func_info(db_info)
    --local name = db_info.name or '[anonymous]'
    return {
...
local function on_hook(event, func_info_id, source)
    ...
    if event == "call" then
        ...
        --LuaProfiler.BeginSample(func_info.id)
