...
public class VertexPainterWindow : OdinEditorWindow
{
    ...
    private PaintingObject[] jobs = Array.Empty<PaintingObject>();
    public bool[] jobEdits = Array.Empty<bool>();
    ...
    private GameObject curCharacterObj;
    ...
    private void InitMeshes()
    {
        ...
        var objs = Selection.GetFiltered(typeof(GameObject), SelectionMode.Editable | SelectionMode.Deep);
        for (int i = 0; i < objs.Length; ++i)
        {
            ...
            if (go != null)
            {
                ...
                if (skinMr != null)
                {
                    if (skinMr != null && skinMr.sharedMesh != null && go.activeSelf)
                    {
                ...
                else
                {
                    ...
                    if (mf != null && r != null && mf.sharedMesh != null && mf.sharedMesh.isReadable && go.activeSelf)
                    {
    ...
    {
        if (curCharacterObj == null)
            return;

        var characterName = curCharacterObj.name;
        if (characterName.Contains("("))
        {
            EditorUtility.DisplayDialog("错误", "角色预设名称不规范!", "关闭");
            return;
        }

        //创建文件夹
        var fullName = $"Assets/Raw/CharacterOutLine/{curCharacterObj.name}";
        ...
        foreach (var obj in jobs)
        {
            if (obj.stream != null)
            {
                ...
                {
                    var path = $"{fullName}/{obj.meshFilter.gameObject.name}_vc.asset";
                    var vertexCols = CreateInstance<MeshVertexColor>();
                    vertexCols._colors = obj.stream.colors;
                    AssetDatabase.CreateAsset(vertexCols, path);
                    EditorUtility.SetDirty(vertexCols);
                    ...
                    paintingData.Apply();
                    DestroyImmediate(paintingData);
                    EditorSceneManager.MarkSceneDirty(UnityEngine.SceneManagement.SceneManager.GetActiveScene());
                }
                else
                {
                    var path = $"{fullName}/{obj.tempGo.name}_vc.asset";
                    var vertexCols = CreateInstance<MeshVertexColor>();
                    vertexCols._colors = obj.stream.colors;
                    AssetDatabase.CreateAsset(vertexCols, path);
                    EditorUtility.SetDirty(vertexCols);
    ...
    void OnGUI()
    {
        ...
        {
            EditorGUILayout.LabelField("请选择角色根节点");
            
            curCharacterObj = null;
            enabled = false;
            return;
        }
    
        // if (Selection.activeGameObject.GetComponent<CharacterShaderHelper>() == null)
        // {
        //     EditorGUILayout.LabelField("请选择角色根节点,必须包含 CharacterShaderHelper 组件");
        //
        //     curCharacterObj = null;
        //     enabled = false;
        //     return;
        ...
        curCharacterObj = Selection.activeGameObject;
        ...
        for (int i = 0; i < j.verts.Length; ++i)
        {
            ...
            if (dist < bz)
            {
                if (showPoint)
                {
                    ...
                    Handles.DotHandleCap(0, wp, Quaternion.identity, HandleUtility.GetHandleSize(wp) * 0.03f,
                        EventType.Repaint);
                }
