...
namespace GPUInstancer
{
    public static class GPUInstancerUtility
    {
        ...
        public static void InitializeGPUBuffer<T>(T runtimeData) where T : GPUInstancerRuntimeData
        {
            ...
            {
                Debug.LogError("instance prototype with an empty LOD list detected. There must be at least one LOD defined per instance prototype.== " + runtimeData.prototype.name);
                return;
            }
        ...
        public static GPUInstancerPrefabPrototype GeneratePrefabPrototype(GameObject go, bool forceNew, bool attachScript = true)
        {
            ...
            {
                var basePath = Path.Combine(Path.GetDirectoryName(SceneManager.GetActiveScene().path) ?? string.Empty ,GPUInstancerConstants.PROTOTYPES_PREFAB_PATH);
                string assetPath = basePath + prototype.name + ".asset";
